...
```{r}
payments |>
  filter(str_detect(Vendor, regex("George", ignore_case = TRUE))) |>
  group_by(Vendor) |>
  summarise(count = n()) |>
  arrange(desc(count))
```
...
pg_schools |>
  ...
  summarise (
    total_payments = sum(Amount, na.rm = TRUE)
  ) |>
...
umd_capital <- payments |>
  filter(str_detect(`Agency Name`, regex("University of Maryland", ignore_case = TRUE)) &
         str_detect(`Project Description`, regex("Capital", ignore_case = TRUE))) |>
  group_by(Vendor, `Vendor Zip`) |>
...
```{r}
payments |>
  filter(str_detect(`Project Description`, regex("stem cell", ignore_case = TRUE))) |>
  distinct(`Project Description`)
```
