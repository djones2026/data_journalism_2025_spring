...
md_gov_combined <- md_gov_county 
  inner_join(md_gov_county, md_gov_18, by = "County")

    Cox_pct = ('Cox'/ total_votes) * 100
    Hogan_pct = ('Hogan'/ total_votes_18) * 100
write_csv(md_gov_combined, "data/md_gov_combined.csv")

  mutate(
    total_votes = 'Cox' + 'Hogan',
    moore_pct = ('Moore' / total_votes) * 100,
    hogan_pct = ('Hogan' / total_votes) * 100
  )
write.csv(md_gov_combined, "data/md_gov_combined.csv")
...
# Read the overdose calls data
overdose_calls <- read_csv("data/montgomery_911_overdose_2022.csv")

# Inspect column names to identify zip code and call count columns
colnames(overdose_calls)

# Assuming you have columns like ZipCode and Calls
# Calculate total calls to get percentage
total_calls <- sum(overdose_calls$Calls, na.rm = TRUE)

# Create percentage column
overdose_calls <- overdose_calls %>%
  mutate(
    pct_calls = (Calls / total_calls) * 100
  )
write_csv(overdose_calls, "data/montgomery_911_overdose_pct.csv")
