...
titles <- webpage %>%
  html_elements(".press-release-title") %>%
  html_text()
dates <- webpage %>%
  html_elements(".press-release-date") %>%
  html_text() %>%
  mdy()
press_releases <- tibble(date = dates, title = titles)
```
...
**What type of sanction has the highest amount?**

```{r}
# Scrape Van Hollen's press releases 
...
dates <- webpage %>%
  ...
  mdy()
urls <- webpage %>% 
  html_elements(".press-release-title a") %>%
  html_attr("href")
press_releases <- tibble(date = dates, title = titles, url = urls)
```
