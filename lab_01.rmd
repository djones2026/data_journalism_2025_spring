...
department_mean
group_by(Department)
summarise(MeanSeats = mean(Seats))
arrange(desc(MeanSeats)
```
...
```{r}
group_by(instructors, deparment)
summarise(title = n())
arrage(desc(title))
```
...
**It would have been easier to find the answer if the dataset had the total number of courses each instructor has taught.**
...
```{r}
group_by(department)
summarise(meanseats = mean(Seats, na = TRUE))
arrange(desc(MeanSeats))
```
