...
**Answer:the yearswitched from the right to left and has dashes**
...
**The dates are all on the 1st up until November, but their not in order by the month.**
...
**I think the code is adding the votes of each canidates**
...
**The answer changed because it's now in descending order**
...
**This was more useful because I can see the votes and candidates from each county. **
...
**I think the mutate statement is converting the full state name to the abbreviation and associating the appropriate address to the right state**
...
**Yes it does because it's coded to be grouped in descending order by state and that's what the code shows**
...
1.  **How could you filter data to only show only morning Tuesday and Thursday classes**

2.  **Is there a way we can filter data to only show classes on McKeldin Mall? **

3.  **Can you mutate the number of seats a class has? **
