...
```{r}
title <- umd_courses |> filter(department == "Climate")
head(title)

courses_2025 <- umd_courses |> filter(term > 202500)
head(courses_2025)

selected_courses_2025 <- umd_courses |> select(id, title)
head(selected_courses_2025)

climate_seats_1 <- umd_courses |> filter(title == "Climate") |> filter(seats >= 1)
nrow(climate_seats_1)

**0**
```

**Q2.** Of the departments listed in A1, which one is the most interesting to you, and why? **None of the departments I coded had the title CLimate in the course.**
...
```{r}
term <- umd_courses |> filter(term == "Fall 2024")
...
```{r}
maryland_expenses |>
  mutate(address = spending_location(address, "In_state")) |>
  group_by(address) |>
  summarise (total_amount = sum(amount)) |>
  arrange(desc(total_amount))
```

**Q6.** Choose your own adventure: using either the UMD courses or campaign expenses data, ask and answer a question that is different than the previous ones. You can use one of the ones you provided in the pre_lab. **How could you filter data to only show morning Tuesday and Thursday classes**
